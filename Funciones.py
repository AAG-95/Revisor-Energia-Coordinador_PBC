...
class ObtencionDatos:
    """
    Esta clase contiene funciones para extraer y procesar datos de hojas de cálculo de Excel.
    ...
    def obtencion_Tablas(self, data_total, primera_fila, primera_columna):
        """
        Extrae filas y columnas relevantes de un DataFrame a partir de la fila y columna especificadas.
        Seleccionar primera fila y primera columna. Se rcorrer las filas y columnas hasta encontrar una fila y columnas con todos los valores nulos.

        Atributos:
        - data_total: Un DataFrame de pandas que contiene los datos a extraer.
        - primera_fila: Un entero que representa el índice de la primera fila a extraer.
        - primera_columna: Un entero que representa el índice de la primera columna a extraer.

        Devuelve:
        - Un DataFrame de pandas que contiene las filas y columnas extraídas.
        """
        # Extraer las filas y columnas relevantes de la hoja de cálculo
        ...
        for i, value in enumerate(first_row):
            ...
            if value in value_counts:
                value_counts[
                    value
                ] += 1  # Incrementa el contador y modifica el valor actual
                first_row.loc[i] = (
                    f"{value}_{value_counts[value]}"  # Agregar un sufijo al valor actual
                )
            else:
                value_counts[value] = (
                    1  # Si es la primera aparición, registra el valor en el diccionario
                )
    ...
    def obtencion_tablas_clientes(
        self, data_total, primera_fila, primera_columna, ultima_columna
    ):
        """
        Extrae filas y columnas relevantes de un DataFrame a partir de la fila y columna especificadas.
...
class ProcesamientosDeDatos:
    """
    Esta clase contiene funciones para procesar y guardar datos en archivos CSV.
    """

    # Función para procesar datos y guardarlos en archivos CSV
    def process_data(self, carpeta_salida, dataframes, titulo, par):
        """
        Combina una lista de DataFrames y los guarda en un archivo CSV.

        Atributos:
        - carpeta_salida: Ruta de la carpeta donde se guardarán los archivos CSV.
        - dataframes: Lista de DataFrames de pandas a combinar.
        - titulo: Nombre base del archivo CSV de salida.
        - par: Parámetro adicional para agregar al nombre del archivo.

        Devuelve:
        - None (archivo se carga en carpeta como csv)
        """
        ...
        # Verificar si el parámetro es una cadena o una tupla
    ...
    def combinar_y_guardar_csv(self, lista_nombres_archivos, directorio, lista_meses):
        ...
        Devuelve:
            None (archivos CSV combinados se guardan en la carpeta especificada).
        """
        ...
        for nombre_archivo in lista_nombres_archivos:
            ...
            # Si se encontraron DataFrames, combinarlos y guardar el resultado en un archivo CSV
            if dataframes:
    ...
    def guardar_en_excel(dataframes, output_file):
        """
        Guarda una lista de DataFrames en un archivo de Excel.

        Atributos:
        - dataframes: Lista de tuplas que contienen el nombre de la hoja y el DataFrame correspondiente.
        - output_file: Ruta del archivo de Excel de salida.

        Devuelve:
        - None (archivo de Excel se guarda en la ruta especificada).
        """

        # Crear un escritor de Excel
        excel_writer = pd.ExcelWriter(output_file, engine="openpyxl")
        # Escribe cada DataFrame en una hoja de Excel
...
class ConversionDatos:
    """
    Esta clase contiene funciones para convertir datos entre diferentes formatos.
    """

    def convertir_fecha(self, Mes):
        """
        Convierte una cadena de texto en formato "MesAño" a un objeto datetime.

        Atributos:
        - Mes: Una cadena de texto en formato "MesAño" (por ejemplo, "Ene2023").

        Devuelve:
        - Un objeto datetime que representa el primer día del mes y año especificados.
        """
        months = {
            "Ene": 1,
            "Feb": 2,
    ...
    def convertir_fecha_numeral(self, Mes):
        """
        Convierte una cadena de texto en formato "AñoMes" a un objeto datetime.

        Atributos:
        - Mes: Una cadena de texto en formato "AñoMes" (por ejemplo, "2301").
    ...
    def generar_pares(
        ...
        """
        Genera una lista de pares de años y meses entre los años y meses especificados.

        Atributos:
        - primer_año: El primer año en el rango.
        - último_año: El último año en el rango.
        - primer_mes_primer_año: El primer mes del primer año en el rango.
        - último_mes_último_año: El último mes del último año en el rango.

        Devuelve:
        - Una lista de pares de años y meses en el formato (Año, AñoMes).
        """
        pares_lista = []
    ...
    def generar_listado_meses(
        ...
        """
        Función para generar una lista de meses en formato "YYMM" entre los años y meses especificados.
        ...
        Devuelve:
        - Una lista de meses en formato "YYMM".
        """
        pares_lista = []
    ...
    def años_y_meses(self, lista_meses):
        """
        Determina los primeros y últimos años y meses en una lista de meses.

        Atributos:
        - lista_meses: Una lista de meses en formato "YYMM" (por ejemplo, ["2106", "2107"]).

        Devuelve:
        - Una tupla con el primer año, último año, primer mes y último mes.
        """
