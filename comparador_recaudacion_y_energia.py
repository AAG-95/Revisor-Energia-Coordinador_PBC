...
for par in pares_lista:
    ...
    df_combinado["Energía Declarada [kWh]"] = df_combinado["Energía Declarada [kWh]"].astype(str).str.replace(',', '.').astype(float)
    ...
    df_combinado["% Diferencia Energía"] = (df_combinado["Energía Balance [kWh]"] - df_combinado["Energía Declarada [kWh]"])/ df_combinado["Energía Balance [kWh]"]
    ...
    df_combinado["Tipo"] = df_combinado.apply(lambda x: "Clave Obsoleta" if pd.isna(x["Energía Balance [kWh]"]) or x["Energía Balance [kWh]"] == 0 else ("Clave no informada en RCUT" if pd.isna(x["Diferencia Energía [kWh]"]) else ("Diferencia Energía con Diferencias" if x["% Diferencia Energía"] > 0.05 else "Diferencia Energía sin Diferencias")), axis=1)

    # Split
    df_combinado["Barra"] = df_combinado["Barra-Clave-Suministrador-Mes"].str.split("-_-", expand=True)[0]
    df_combinado["Clave"] = df_combinado["Barra-Clave-Suministrador-Mes"].str.split("-_-", expand=True)[1] 
    df_combinado["Suministrador"] = df_combinado["Barra-Clave-Suministrador-Mes"].str.split("-_-", expand=True)[2]
