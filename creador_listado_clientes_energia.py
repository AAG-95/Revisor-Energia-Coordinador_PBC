...
#! Main Program
    def get_dataframes(self):
        ...
        # Get dataframe from self.ruta_homologa_propietarios sheet 'Homologa'
        self.df_homologa_propietarios = pd.read_excel(
            self.ruta_homologa_propietarios, sheet_name="Homologa"
        )

        # ? Get Versión
        # Get dataframe from self.ruta_control_versiones sheet 'Versiones'
...
# List of sheets to read from ZIP file
    ...
    def procesar_meses(self,mes):
            ...
            with zipfile.ZipFile(ruta_zip) as myzip:
                for i in self.lista_balance_fisico:
                    ...
                    with myzip.open(ruta_correcta + i + mes_numeral + ".xls") as myfile:
                        ...
                        # Merge dataframe df_clientes with self.df_homologa_propietarios based in column Propietario from both df
            ...
            # Get database from self.ruta_registro_cambios_empresas
            ...
            if mes_fecha not in list(df_registro_cambios_empresas["Mes"].unique()):
                ...
                # Rewrite df_registro_cambios_empresas_final into self.ruta_registro_cambios_empresas
            ...
            if os.path.isfile(self.ruta_retiros_historicos_L):
                df_historico_clientes_L = pd.read_csv(
                    self.ruta_retiros_historicos_L, sep=";", encoding="UTF-8"
                )
                ...
                if mes_fecha not in df_historico_clientes_L["Mes"].unique().tolist():
                    ...
                    df_retiros_historico_L_final.to_csv(
                        self.ruta_retiros_historicos_L, sep=";", index=False
                    )
                    ...
                    df_registro_cambios_clientes_L.to_csv(
                        self.ruta_registro_cambios_clientes_L, sep=";", index=False, encoding="UTF-8"
                    )
                else:
                    print(
            ...
            if os.path.isfile(self.ruta_retiros_historicos_R):
                df_historico_clientes_R = pd.read_csv(
                    self.ruta_retiros_historicos_R, sep=";", encoding="UTF-8"
                )
                ...
                if mes_fecha not in df_historico_clientes_R["Mes"].unique().tolist():
                    ...
                    df_retiros_historico_R_final.to_csv(
                        self.ruta_retiros_historicos_R, sep=";", index=False
                    )
                    ...
                    df_registro_cambios_clientes_R.to_csv(
                        self.ruta_registro_cambios_clientes_R, sep=";", index=False, encoding="UTF-8"
                    )
                else:
                    print(
