...
class DataProcessor:
    def __init__(self, primer_año, primer_mes_primer_año, último_año, último_mes_último_año):
    ...
    def procesamiento_datos(self):
        ...
        self.lista_nombre_archivos = ["BDD Clientes Libres Históricos.csv", "BDD Clientes Regulados Históricos.csv", "BDD Observaciones Históricas.csv", "BDD Revisor Clientes L Históricos.csv", "BDD Revisor Clientes R Históricos.csv"]
        ...
        for idx, nombre_archivo in enumerate(self.lista_nombre_archivos):
            ...
            if os.path.isfile(carpeta_entrada + "Revisión Histórica\\"+ nombre_archivo):
                self.lista_df_historicos[idx] = pd.read_csv(carpeta_entrada + "Revisión Histórica\\"+ nombre_archivo, sep=";", encoding = "UTF-8", header=0 )
            
                self.lista_df_historicos[idx]["mes_repartición"] = pd.to_datetime(self.lista_df_historicos[idx]["mes_repartición"])
                valores_mes = self.lista_df_historicos[idx]["mes_repartición"].dt.strftime('%d-%m-%Y').unique()
        ...
        for idx, (lista_dataframe, nombre_archivo) in enumerate(zip(lista_dataframes_mes_analizado, self.lista_nombre_archivos)):
            ...
            for i, mes_rep in zip(lista_dataframe, meses_rep):
                ...
                if mes_df in lista_valores_mes[idx] or df_vacio:
                    ...
                    self.lista_df_historicos[idx] = pd.concat([self.lista_df_historicos[idx], i])
    ...
    def actualizador_recaudacion_historica(self):
        ...
        for variable, nombre_archivo in zip(self.lista_df_historicos, self.lista_nombre_archivos):
            variable.to_csv(carpeta_salida + nombre_archivo, sep=";", encoding="UTF-8", index=False)

# Usage
processor = DataProcessor(2020, 8, 2020, 10)
